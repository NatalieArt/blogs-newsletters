...
- Finish the Agent screenshot with one clean outer contour: the screenshot wrapper owns the one-pixel translucent mint border, rounded clipping, and restrained shadow. Keep the four-pixel dark gradient inset inside that outer border. Remove the child image's separate border and shadow, and use a slightly smaller concentric radius on the image so no decorative layer can appear outside the visible frame. Apply the same finish on the root-index feature for visual continuity.
