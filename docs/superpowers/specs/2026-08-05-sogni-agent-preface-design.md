...
- Present the Agent screenshot without a decorative frame: no mint border, gradient inset, wrapper padding, or shadow. The screenshot wrapper should provide only ten-pixel rounded clipping, while the image fills it edge to edge with the same radius. Apply the same minimal finish on the root-index feature for visual continuity.
